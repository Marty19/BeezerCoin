...
const string strMessageMagic = "BeezerCoin Signed Message:\n";
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in BeezerCoin
    bool fStrictPayToScriptHash = true; // Always active in BeezerCoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BeezerCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        nStakeTargetSpacing = 10 * 60; // test block spacing is 10 minutes
    }
...
string GetWarnings(string strFor)
{
    ...
    if (Checkpoints::IsSyncCheckpointTooOld(60 * 60 * 24 * 365 * 10) && !fTestNet && !IsInitialBlockDownload())
    {
        nPriority = 100;
...
unsigned char pchMessageStart[4] = { 0xab, 0xd4, 0xc1, 0xbd };
